...
export const Dashboard: React.FC = () => {
  ...
  const { data: lowStock } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await api.get(`/inventory/alerts/low-stock/?warehouse_id=${warehouseId}`);
  ...
  const { data: nearExpiry } = useQuery({
    ...
    queryFn: async () => {
      if (!warehouseId) return { count: 0 };
      const res = await api.get(`/inventory/alerts/near-expiry/?warehouse_id=${warehouseId}&days=30`);
      return { count: res.data.length }; // Assuming list response
    },
    enabled: !!warehouseId,
  });
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{lowStock?.count || 0}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{nearExpiry?.count || 0}</div>
          </CardContent>
        </Card>
      </div>
