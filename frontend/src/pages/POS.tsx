...
export const POS: React.FC = () => {
  ...
  const createSaleMutation = useMutation({
    ...
    onSuccess: (data) => {
      setLastReceipt(data); // Assuming backend returns receipt data or ID
      setCart([]);
  ...
  const handleCheckout = () => {
    ...
    const payload = {
      ...
      discount_total: "0.00",
      payments: [{ method: paymentMethod, amount: total.toFixed(2) }] // Full payment for MVP
    };
      ...
      <Dialog open={isPaymentOpen} onOpenChange={setIsPaymentOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCheckout}>{t('confirm')}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
